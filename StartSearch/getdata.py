...
def update_data_in_google_sheet(sheet_id, doc_range, values):
    ...
    r = sheet_service.spreadsheets().values().update(spreadsheetId=sheet_id, valueInputOption=value_input_option, range=doc_range, body=value)
    return r.execute()
...
def parse_data(data, meta):
    ...
    if meta:
        return {
            result[0][0]: [value[0] for index, value in enumerate(result) if index != 0 and value[0] != ''],
            result[0][1]: [value[1] for index, value in enumerate(result) if index != 0 and value[1] != ''],
            result[0][2]: [value[2] for index, value in enumerate(result) if index != 0 and value[2] != '']
        }
    else:
        return {
            result[0][0]: [value[0] for index, value in enumerate(result) if index != 0 and value[0] != ''],
